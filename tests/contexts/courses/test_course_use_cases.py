...
def test_create_new_course_with_invalid_duration(invalid_value):
    ...
    with pytest.raises(ValueError):
        CourseCreator(repository).execute(str(uuid4()), "test", invalid_value)
...
def test_create_new_course_with_invalid_title(invalid_value):
    ...
    repository.clear()
    with pytest.raises(ValueError):
        CourseCreator(repository).execute(str(uuid4()), invalid_value, 60)
...
def test_should_raise_error_when_title_already_exists():
    repository = InMemoryRepository()
    repository.clear()
    CourseCreator(repository).execute(str(uuid4()), "test", 60)
